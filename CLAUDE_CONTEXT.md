...
## Этап 6 — Борьба с обрезкой генерации (ROOT-CAUSE для "сырых" проектов) ✅

**Диагноз (на живых данных).** Проекты выглядели "сыро" и неадаптивно не из-за слабых промптов, а из-за ОБРЕЗКИ ответа модели. Генерация index.html+style.css+script.js шла одним ответом с лимитом max_tokens: 16384 (потолок gpt-4o). На больших проектах ответ обрывался: хвост style.css (где @media) терялся, а script.js целиком заменялся generic-фолбэком санитайзера. Подтверждено замером проекта id=5: script.js=583б (фолбэк AOS.init), style.css заканчивался авто-инжектом "Hamburger desktop guard" (признак обрыва).

**Доп. находка.** UI создаёт проекты как projectType=landing, поэтому presentation-промпт никогда не применялся к запросам "сделай презентацию".

**Исправления (commit c6a4e0e):**
1. `createCompletionWithContinuation()` в openai.ts — при finish_reason==="length" автоматически делает дозапросы ("continue exactly where you stopped") и склеивает части (до 4 продолжений). Применено в generateWithOpenAI.
2. Авто-определение типа в routes/projects.ts (generate): если тип явно не задан, по ключевым словам (presentation/slide/deck/презентаци/слайд) выбирается projectType=presentation.

**Замер ДО → ПОСЛЕ (живая генерация, gpt-4o + E2B):**
| Метрика | ДО (id=5) | ПОСЛЕ (id=6) |
|---|---|---|
| script.js | 583 (фолбэк-стаб) | 1083 (реальная навигация по слайдам) |
| style.css | 7095 (обрыв) | 9652 (полный) |
| index.html | 3798 | 4764 |
| Truncation-guard инжектнут | да | НЕТ |
| presentation-промпт применён | нет (landing) | да |

Проверки ПОСЛЕ: JS has slide nav=true, JS generic fallback=false, CSS @media=7, truncation-guard injected=FALSE, CSS заканчивается полным валидным правилом. Сборка (build) и detection — без ошибок.
...
| `(pending)` | Add sanitizeImages post-processor: fix broken img tags on all generated sites |
...
- **Не ломать:**
  - Строгий JSON-формат ответа OpenAI: `{ files: [{path, content}], message }` — парсер устойчив к обрыву: `parseGeneratedOutput` пробует `JSON.parse`, при сбое запускает `recoverPartialFiles` (state-machine по bytes) и возвращает уже полные файлы; throw только если совсем ничего не извлечь. После парсинга всегда вызывается **шестерная** детерминированная нулевого-токен пост-обработка: `sanitizeScripts(sanitizeFonts(sanitizeImages(sanitizeNavbar(sanitizeMobile(sanitizeAosInit(files))))))`:
    - `sanitizeScripts`: (outermost) если `script.js` отсутствует, пуст или не содержит ни `hamburger`, ни `DOMContentLoaded` — инжектирует минимальный фолбэк (hamburger-тоггл + smooth scroll + `window.AOS && AOS.init({once:true})`) и добавляет `<script src="script.js" defer></script>` перед `</body>` в index.html. Идемпотентна: повторный вызов ничего не дублирует. Тесты: `sanitizeScripts.test.ts` ✅.
    - `sanitizeFonts`: парсит `--font-*` переменные из `:root` в style.css; для каждой семьи, не покрытой существующими `fonts.googleapis.com` `<link>` в index.html — инжектирует недостающую `<link>`. Идемпотентна. Тесты: `sanitizeFonts.test.ts` (20 кейсов ✅).
    - `sanitizeNavbar`: (1) Contrast guard — если navbar имеет `backdrop-filter` или `background:transparent` без `:not(.scrolled)` правил — дописывает `color:#fff` на nav-links/logo в начальном состоянии; (2) Mobile guard — всегда дописывает `@media(max-width:768px){ header,navbar{flex-direction:row!important;} + header .btn{width:auto!important} }` если маркер отсутствует. Идемпотентна. Тесты: `sanitizeNavbar.test.ts` (19 кейсов ✅).
    - `sanitizeAosInit`: (1) в `script.js` обнаруживает `AOS.init()` обёрнутый в if-блок и переписывает на безусловный вызов с `duration: prefersReduced ? 0 : 700`, (2) в `style.css` дописывает `[data-aos]:not(.aos-init) { opacity:1!important }` если отсутствует.
    - `sanitizeMobile`: (1) в `index.html` инжектирует `<meta name="viewport" ...viewport-fit=cover>` если нет, (2) в `style.css` дописывает минимальный `@media (max-width:768px)` блок (hamburger, 1-col grids, кнопки 100%, overflow-x:hidden) если нет ни одного `@media (max-width:`. Обе идемпотентны, при отсутствии бага — ничего не меняют.
    - `sanitizeImages`: применяется только к `*.html` файлам; для каждого `<img>` тега: (1) заменяет `source.unsplash.com` → `images.unsplash.com`, (2) добавляет `onerror="this.onerror=null;this.src='https://loremflickr.com/1200/800/{keyword}'"` к unsplash-изображениям без fallback (keyword = первое слово alt, fallback — "photo"), (3) добавляет `loading="lazy"` если нет (`loading="eager"` на hero НЕ трогает), (4) добавляет `alt="Photo"` если атрибут отсутствует или пустой. Идемпотентна: повторный вызов ничего не меняет. Тесты: `artifacts/api-server/src/lib/__tests__/sanitizeImages.test.ts` (23 кейса).
  - Модель `gpt-4o`, `temperature: 0.2`, `max_tokens: 16384` (абсолютный максимум gpt-4o/gpt-4o-mini; генерация сайта — streaming и non-streaming и editProject); `generatePlan` = 1000, `generateZeusMd` = 600 — не менять.
...
**Детерминированный пост-процессор `sanitizeScripts` + [SCRIPTS] правило в промпте**

**Что изменено:**
- `artifacts/api-server/src/lib/openai.ts`:
  - SYSTEM_PROMPT — добавлен пункт `[SCRIPTS]` в раздел INTERACTIVITY: требует всегда включать непустой script.js с (a) hamburger-тогглом, (b) smooth scroll по якорям, (c) AOS.init({once:true}) внутри DOMContentLoaded.
  - Добавлена `export function sanitizeScripts()` — outermost guard: если script.js отсутствует / пуст / не содержит ни `hamburger`, ни `DOMContentLoaded` — инжектирует минимальный фолбэк и гарантирует `<script src="script.js" defer></script>` перед `</body>`. Идемпотентна.
  - Цепочка стала **шестерной** в обоих путях `parseGeneratedOutput`: `sanitizeScripts(sanitizeFonts(sanitizeImages(sanitizeNavbar(sanitizeMobile(sanitizeAosInit(files))))))`.
- `artifacts/api-server/src/lib/__tests__/sanitizeScripts.test.ts` — 24 unit-теста ✅ (отсутствующий/пустой/слабый script → фолбэк; рабочий → не меняется; двойной прогон не дублирует `<script>`).
- E2E тест «Zevs Barbershop» (power tier): 10/10 проверок ✅ — script.js непустой, hamburger-тоггл присутствует, AOS.init есть, `<script src="script.js">` в index.html, шрифты/navbar/mobile не сломались.

### Предыдущий выполненный шаг
**Fix 3 E2E defects: fonts, navbar contrast, mobile header** (sanitizeFonts + sanitizeNavbar + SYSTEM_PROMPT hardening)

**Что изменено:**
- `artifacts/api-server/src/lib/openai.ts`:
  - SYSTEM_PROMPT — усилено правило TYPOGRAPHY (требование отдельного `<link>` на каждую семью шрифтов); раздел 9 MICRO-INTERACTIONS расширен правилом CONTRAST для navbar; блок `@media (max-width:768px)` исправлен (`header nav` убран из flex-column, добавлены явные исключения для header + btn); QUALITY BAR — 3 новых чекбокса.
  - MOBILE_CSS_FALLBACK — добавлены `header{flex-direction:row!important}` + `header .btn{width:auto!important}` overrides.
  - Добавлены `sanitizeFonts()` и `sanitizeNavbar()`. Цепочка стала пятерной: `sanitizeFonts(sanitizeImages(sanitizeNavbar(sanitizeMobile(sanitizeAosInit(files)))))`.
- `artifacts/api-server/src/lib/__tests__/sanitizeFonts.test.ts` — 20 unit-тестов ✅.
- `artifacts/api-server/src/lib/__tests__/sanitizeNavbar.test.ts` — 19 unit-тестов ✅.
- E2E тест «Zevs Barbershop» (power tier): 10/10 проверок ✅ — оба шрифта имеют GF links, navbar-guard инжектируется, header остаётся строкой на mobile.

### Предыдущий выполненный шаг (2 уровня назад)
**Детерминированная пост-обработка изображений `sanitizeImages`**

**Что изменено:**
- `artifacts/api-server/src/lib/openai.ts` — добавлен `sanitizeImages()`, вшит в `parseGeneratedOutput` и `recoverPartialFiles` как третий шаг цепочки.
- `SYSTEM_PROMPT` — правило IMG RULES #4 исправлено (loremflickr onerror вместо picsum), добавлены два img-чекбокса в QUALITY BAR.
- `artifacts/api-server/src/lib/__tests__/sanitizeImages.test.ts` — 23 unit-теста (все прошли ✅).
- E2E тест «Zevs Barbershop» (lite tier): 38/38 ✅ — все img unsplash имеют onerror+loremflickr, loading, non-empty alt; нет source.unsplash.com.
...
- В конце style.css — сначала CSS-fallback, потом @media:
  ```css
  /* AOS CDN failure fallback: keeps content visible if AOS JS fails to load.
     AOS adds .aos-init to each element it processes; without it elements stay visible. */
  ...
  ```
- В script.js AOS.init() и lucide.createIcons() вызываются **БЕЗУСЛОВНО** (никогда не внутри if):
  ```js
  const prefersReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  AOS.init({ duration: prefersReduced ? 0 : 700, easing: 'ease', once: true, offset: 80 });
  lucide.createIcons();
  ```
  ⚠️ НЕЛЬЗЯ оборачивать AOS.init() в if — если не вызвать, все [data-aos] остаются opacity:0 и сайт невидим.
...
- JSON `{ files: [{path, content}], message }` — парсер устойчив к обрыву (recoverPartialFiles); max_tokens сайта = 16384
