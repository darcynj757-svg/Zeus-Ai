...
### E2E тест M1–M6 (проект id=8, «SpiceHouse» shop, tier=power)

| Проверка | Результат | Примечание |
|---|---|---|
| `deployError` | `null` ✅ | |
| `previewUrl` HTTP | **200** ✅ | |
| M2 `clamp()` в CSS | **18×** ✅ | M2 работает |
| M3 `auto-fit` в CSS | **3×** ✅ | M3 работает |
| M4 mobile CTA HTML | `<div class="mobile-cta">` ✅ | класс иной, функция верна |
| M4 `position:fixed; bottom:0` | ✅ | |
| M1 `viewport-fit=cover` | ⚠️ не добавил | модель пропустила мета-тег |
| M1 `env(safe-area-inset` | ⚠️ нет | |
| M5 `aspect-ratio` | ⚠️ нет | |
| M6 `min-height:44px` | ⚠️ нет | |
| M6 `font-size:16px` inputs | ⚠️ нет | |
| R1 `@media` брейкпоинтов | **4** ✅ (481/768/1024/reduced-motion) | |
| R2 preconnect теги | **5** ✅ | |
| R2 imgs w+h | **16/16** ✅ | |
| R3 `:focus-visible` | ✅ | |
| R4 `prefers-reduced-motion` | ✅ | |
| R4 `prefersReduced` в JS | ✅ | |

> **Вывод:** Новый SYSTEM_PROMPT работает — M2/M3 и все R1–R4 применяются стабильно. M1/M5/M6 — частичные пропуски модели (очень длинный prompt, контекст). При следующем улучшении можно вынести M1 viewport-meta и M6 16px/44px в более ранний раздел DESIGN SYSTEM.
