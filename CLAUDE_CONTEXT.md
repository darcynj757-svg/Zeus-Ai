...
- **Не ломать:**
  - Строгий JSON-формат ответа OpenAI: `{ files: [{path, content}], message }` — парсер устойчив к обрыву: `parseGeneratedOutput` пробует `JSON.parse`, при сбое запускает `recoverPartialFiles` (state-machine по bytes) и возвращает уже полные файлы; throw только если совсем ничего не извлечь. После парсинга всегда вызывается `sanitizeAosInit(files)` — детерминированная нулевого-токен пост-обработка, которая (1) в `script.js` обнаруживает `AOS.init()` обёрнутый в `if (prefersReduced)` / `if (!prefersReduced)` и переписывает на безусловный вызов `AOS.init({ duration: prefersReduced ? 0 : 700, ... })` + `lucide.createIcons()`, (2) в `style.css` дописывает `[data-aos]:not(.aos-init) { opacity:1!important }` если отсутствует. Идемпотентна, при отсутствии бага — нет изменений.
