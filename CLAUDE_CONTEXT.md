...
**Этап 3, шаг 1 — Роутинг моделей** (коммит «Add model routing with Lite and Power tiers»)

**Что сделано:**
- `artifacts/api-server/src/lib/openai.ts`: добавлены `MODELS = { lite: "gpt-4o-mini", power: "gpt-4o" }` и тип `ModelTier`; все 5 функций (`generateWithOpenAI`, `streamWithOpenAI`, `generatePlan`, `generateZeusMd`, `editProject`) принимают `tier: ModelTier`; `generateZeusMd` и `editProject` defaultят на `"lite"`, остальные на `"power"`
- `artifacts/api-server/src/routes/projects.ts`: добавлена `parseTier(value): ModelTier`; эндпоинты `/generate`, `/edit`, `/plan` читают `tier` из request body и передают в OpenAI функции; логируют `model selected` с именем модели
- `artifacts/vibe-coding/src/pages/home.tsx`: тип `ModelTier`, хук `useState<ModelTier>("power")` в `ChatPanel`; UI-переключатель ⚡ Lite / 🔥 Power в шапке чата рядом с Plan Mode; `onGenerate` и `onEdit` сигнатуры обновлены с `tier` параметром; `handleGenerate` и `handleEdit` в `Home` отправляют `tier` в тело запроса

**Живые тесты пройдены:**
- A) Power → VoltGym лендинг: `tier=power` → `model: gpt-4o` → `files: [index.html, script.js, style.css]` → E2B 200
- B) Lite → патч «сделай заголовок крупнее»: `tier=lite` → `model: gpt-4o-mini` → `patchedFiles: ['style.css']` (1 из 3) → E2B 200

### Следующий шаг — Этап 3, шаг 2 (Чекпоинты)

**Задача: Чекпоинты / снапшоты** — возможность откатить проект к предыдущей версии:
- Новая таблица `snapshots` (id, projectId, filesJson, label, createdAt)
- `POST /api/projects/:id/snapshot` — создать снапшот вручную
- `POST /api/projects/:id/restore/:snapshotId` — восстановить снапшот (upsert файлов, деплой)
- `GET /api/projects/:id/snapshots` — список снапшотов
- Авто-снапшот перед каждой генерацией и edit (если есть файлы)
- UI: кнопка «История» (иконка часы) в шапке `ChatPanel`; дропдаун со списком снапшотов; кнопка «Откат» рядом с каждым
