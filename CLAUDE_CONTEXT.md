...
- [ ] **Итеративное редактирование** — "измени цвет кнопок на красный" патчит ТОЛЬКО затронутый файл, не регенерит всё
...
**Этап 2 Шаг 3 — Надёжность E2B** (`40fc903`)
- `sandbox.ts`: `withRetry` + `DeployError` + 60s `Promise.race`
- `projects.ts`: `onRetry` колбэк → SSE-статус попытки; `deployError` в `done`-событии
- `home.tsx`: `toast.warning` при `deployError`

### Следующий шаг — Итеративное редактирование

**Цель:** инструкция вида "измени цвет кнопок на красный" должна патчить ТОЛЬКО затронутые файлы, не перегенерируя весь проект с нуля.

**Что реализовать:**

1. **`artifacts/api-server/src/lib/openai.ts`** — новая функция:
   ```
   editProject(
     existingFiles: Array<{path: string; content: string}>,
     instruction: string,
     zeusContext?: string | null
   ): Promise<{ files: Array<{path: string; content: string}>; message: string }>
   ```
   - Системный промпт: «Ты редактор кода. Получаешь существующие файлы и инструкцию. Верни ТОЛЬКО изменённые файлы в том же строгом JSON `{ files: [{path, content}], message }`. Не трогай файлы, которые не затронуты инструкцией.»
   - Модель: `gpt-4o`, `temperature: 0.2`, `max_tokens: 16000` — не менять.
   - Если `zeusContext` передан — вшить его в системный промпт чтобы сохранить бренд.

2. **`artifacts/api-server/src/routes/projects.ts`** — новый эндпоинт:
   - `POST /api/projects/:id/edit` (или добавить поле `mode: "edit" | "generate"` в существующий `/generate`)
   - Читает текущие файлы из DB, вызывает `editProject`, мёржит частичный `files[]` с полным набором в DB (upsert по `path`), деплоит, возвращает результат.
   - **Не менять** формат `files[]+message`, деплой E2B, SSE-протокол.

3. **`artifacts/vibe-coding/src/pages/home.tsx`** — UI:
   - Добавить переключатель или автодетект: короткие правки (< ~100 символов?) → режим edit; длинные промпты → обычная генерация.
   - Или явная кнопка «Редактировать» рядом с «Сгенерировать».

**Инварианты, которые НЕЛЬЗЯ нарушать:**
- Строгий JSON `{ files: [{path, content}], message }` — парсер не меняется.
- `gpt-4o`, `temperature: 0.2`, `max_tokens: 16000`.
- E2B: `python3 -m http.server 3000`, без build-шага.
- Бренд-контекст `zeusContext` должен подмешиваться и при edit-режиме.
