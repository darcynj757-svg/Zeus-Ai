...
- [ ] Надёжность деплоя E2B: ретраи, таймауты, fallback-сообщения
...
| pending   | Add zeus.md project memory for brand-consistent iterations |

> Следующий коммит (создаётся платформой): `Add zeus.md project memory for brand-consistent iterations`
...
**Этап 2, Шаг 3** — Выбрать следующий приоритет:
- [ ] **Надёжность E2B** — ретраи (3× с экспоненциальным backoff), таймауты 30s, fallback-сообщения при деплое
- [ ] **Итеративное редактирование** — "измени цвет кнопок на красный" патчит только нужные файлы, не перегенерирует всё
- [ ] **Тип `portfolio`** — 5-й тип: секции «Проекты», «Навыки», «Контакт»
