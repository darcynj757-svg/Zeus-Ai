...
### Этап 2 — Качество
- [ ] **Plan Mode** — Zeus сначала показывает план (секции/компоненты), пользователь утверждает, потом генерирует
...
**Определить с пользователем приоритет Этапа 2:**
- Plan Mode (Zeus показывает структуру перед генерацией)?
- Память проекта `zeus.md` (накопление контекста бренда)?
- Тип `portfolio` как 5-й вариант?
