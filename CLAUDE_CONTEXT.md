...
- [ ] **Память проекта `zeus.md`** — файл внутри проекта с накопленным контекстом (бренд, цвета, предпочтения)
...
> Следующий коммит (создаётся платформой): `Add Plan Mode: preview structure before generation`
...
**Этап 2, Шаг 2** — Выбрать следующий приоритет:
- [ ] **Память проекта `zeus.md`** — накопление контекста (бренд, цвета) между итерациями
- [ ] **Надёжность E2B** — ретраи, таймауты, fallback-сообщения при деплое
