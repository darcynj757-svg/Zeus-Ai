...
- **Не ломать:**
  - Строгий JSON-формат ответа OpenAI: `{ files: [{path, content}], message }` — парсер устойчив к обрыву: `parseGeneratedOutput` пробует `JSON.parse`, при сбое запускает `recoverPartialFiles` (state-machine по bytes) и возвращает уже полные файлы; throw только если совсем ничего не извлечь.
