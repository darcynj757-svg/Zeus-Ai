...
- **Не ломать:**
  - Строгий JSON-формат ответа OpenAI: `{ files: [{path, content}], message }` — парсер устойчив к обрыву: `parseGeneratedOutput` пробует `JSON.parse`, при сбое запускает `recoverPartialFiles` (state-machine по bytes) и возвращает уже полные файлы; throw только если совсем ничего не извлечь. После парсинга всегда вызывается **тройная** детерминированная нулевого-токен пост-обработка: `sanitizeImages(sanitizeMobile(sanitizeAosInit(files)))`:
...
**Детерминированная пост-обработка изображений `sanitizeImages`** (коммит «Add sanitizeImages post-processor: fix broken img tags on all generated sites»)

**Что изменено:**
- `artifacts/api-server/src/lib/openai.ts` — добавлен `sanitizeImages()`, вшит в `parseGeneratedOutput` и `recoverPartialFiles` как третий шаг цепочки: `sanitizeImages(sanitizeMobile(sanitizeAosInit(files)))`.
